...
def save_outputs(img_path, output_file_name):
    ...
    if args.task == 'depth':
        ...
        output_path = img_path.replace(Path(img_path).name, 
                                                Path(img_path).name.replace("rgb", 'normal').replace(Path(img_path).suffix, ''))
...
if img_path.is_dir():
    ...
    images = glob.glob(args.img_path + "/train/*_rgb.exr", recursive=True)
