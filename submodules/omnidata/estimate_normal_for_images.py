...
def save_outputs(img_path, output_file_name):
    ...
    if args.task == 'depth':
        ...
        output_path = img_path.replace(Path(img_path).name, 
                                                Path(img_path).name.replace(Path(img_path).stem, Path(img_path).stem+'_normal').replace(Path(img_path).suffix, ''))
        np.save(os.path.join(args.output_path, output_path), output.to('cpu').numpy())
...
if img_path.is_dir():
    ...
    images = glob.glob(args.img_path + "/images_4/*.JPG", recursive=True)
