...
__device__ void computeCov3D(int idx, const glm::vec3 scale, float mod, const glm::vec4 rot, const float* dL_dcov3Ds, glm::vec3* dL_dscales, glm::vec4* dL_drots, const glm::vec3* dL_dnormal, const float* view)
{
	...
	glm::vec4* dL_drot)
{
	...
	if (scales)
		computeCov3D(idx, scales[idx], scale_modifier, rotations[idx], dL_dcov3D, dL_dscale, dL_drot, (glm::vec3*)dL_dnormal, view);
}
...
renderCUDA(
	...
	const float* __restrict__ dL_dpixels_n,
	...
	)
{
	...
	float accum_rec[C] = { 0 }, accum_rec_n[3] = {0};
	float accum_rec_d = 0.f, accum_rec_o = 0.f, accum_rec_f[24] = { 0 }, accum_rec_v = 0;
	float dL_dpixel[C], dL_dpixel_n[3];
	float dL_dpixel_d, dL_dpixel_o, dL_dpixel_v, dL_dpixel_f[24];
	if (inside)
	{	for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
		for (int i = 0; i < 3; i++)
			dL_dpixel_n[i] = dL_dpixels_n[i * H * W + pix_id];
        dL_dpixel_d = dL_dpixels_d[pix_id] * 1;
	...
	float last_alpha = 0;
	float last_depth = 0;
	float last_color[C] = { 0 }, last_normal[3] = {0}, last_var = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < 3; i++) 
				collected_normal[i * BLOCK_SIZE + block.thread_rank()] = normal[coll_id * 3 + i];
...
//                 }
			...
			if (surface) {
				for (int ch = 0; ch < 3; ch++) { // grads w.r.t. tangent normal	
					...
					float dL_dchannel = dL_dpixel_n[ch], dL_dalpha_normal = 0;
			...
			for (int ch = 0; ch < 1; ch++) { // grads w.r.t. depth
				...
				float dL_dchannel = dL_dpixel_d, dL_dalpha_depth = 0;

				if (normalize_depth) { // Additional gradients w.r.t. alpha from depth normalization
					dL_dchannel /= (1.f - T_final);
					dL_dalpha_depth += dL_dpixel_d * D_final / (1.f - T_final) / (1.f - T_final) * -T_final / (1 - alpha) / T;
				}
			...
			dL_dalpha += dL_dpixel_o * T_final / (1 - alpha);
			...
			if (!normalize_depth) dL_dalpha += (-T_final / (1.f - alpha)) * (10 * dL_dpixel_d);
			...
			if (surface && per_pixel_depth) {
				dL_dNDC.x += 1 * -dL_dpixel_d * (Jinv_u0_u1[6] * Jinv_u0_u1[0] + Jinv_u0_u1[9] * Jinv_u0_u1[2]);
				dL_dNDC.y += 1 * -dL_dpixel_d * (Jinv_u0_u1[6] * Jinv_u0_u1[1] + Jinv_u0_u1[9] * Jinv_u0_u1[3]);
			}
