	...
	)
{
	...
	float C[CHANNELS] = { 0 }, N[3] = { 0 }, F[33] = { 0 }, D = 0, depth_first, CUT = 0, Depth = 0, Opacity = 0;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (surface) for (int ch = 0; ch < 3; ch++) N[ch] += collected_normal[ch * BLOCK_SIZE + j] * weight;
