	...
	const bool debug)
{
  ...
  torch::Tensor out_pseudo_normal = torch::full({3, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		out_pseudo_normal.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(rendered, n_contrib, out_color, out_normal, out_opacity, out_depth, out_feature, out_pseudo_normal, out_surface_xyz, out_weights, radii, geomBuffer, binningBuffer, imgBuffer);
}
