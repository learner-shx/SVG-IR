...
def get_bvh_m():
    m_gen_ele = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/get_elements.slang'))
    m_morton_codes = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/lbvh_morton_codes.slang'))
    m_radixsort = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/lbvh_single_radixsort.slang'))
    m_hierarchy = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/lbvh_hierarchy.slang'))
    m_bounding_box = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/lbvh_bounding_boxes.slang'))

    return m_gen_ele, m_morton_codes, m_radixsort, m_hierarchy, m_bounding_box

def get_intersection_m():
    m_intersect_test = slangtorch.loadModule(os.path.join(current_dir, 'bvhworkers/intersect_test.slang'))
    return m_intersect_test
...
def get_gs_bvh(centers, scales, rotates, m_gen_ele, m_morton_codes, m_radixsort, m_hierarchy, m_bounding_box):
    #first part, get element and bbox---------------
    primitive_num = centers.shape[0]
    ele_primitiveIdx = torch.zeros((primitive_num, 1), dtype=torch.int).cuda()
    ele_aabb = torch.zeros((primitive_num, 6), dtype=torch.float).cuda()

    # Invoke normally
    m_gen_ele.generateGaussianElements(centers=centers, scales=scales, rotates=rotates, ele_primitiveIdx=ele_primitiveIdx, ele_aabb=ele_aabb)\
        .launchRaw(blockSize=(256, 1, 1), gridSize=((primitive_num+255)//256, 1, 1))
    extent_min_x = ele_aabb[:,0].min()
    extent_min_y = ele_aabb[:,1].min()
    extent_min_z = ele_aabb[:,2].min()
