...
if __name__ == "__main__":
    ...
    for test_dir in tqdm(test_dirs):
        ...
        # gs normal
        normal_gs_path = os.path.join(output_dir, "normal", f"{test_id:05d}.png")
    ...
    # 打开（或创建）一个文本文件用于写入
    with open("tensoir_log.txt", "a") as f:
        # 将数据写入文件
        f.write("MAE Normal Evaluation Results:\n")
        f.write(f"gs: {mae_gs}\n")
        #f.write(f"from_depth: {mae_from_depth}\n")
