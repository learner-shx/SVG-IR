	...
	const torch::Tensor& config)
{
  ...
  torch::Tensor out_vfeature = torch::full({VS/4, H, W}, 0.0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		vfeatures.contiguous().data_ptr<float>(),
		...
		out_vfeature.contiguous().data_ptr<float>(),
		out_weights.contiguous().data_ptr<float>(),
		radii.contiguous().data<int>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, out_normal, out_depth, out_opac, out_feature, out_vfeature, out_weights, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor  , torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& dL_dout_vfeature,
	const torch::Tensor& sh,
	const int degree,
	const torch::Tensor& campos,
	...
	const torch::Tensor& config) 
{
  ...
  torch::Tensor dL_dvfeatures = torch::zeros({P, VS}, vfeatures.options());
  ...
  if(P != 0)
  {  
	  CudaRasterizer::Rasterizer::backward(P, S, VS, degree, M, R,
		background.contiguous().data<float>(),
		W, H, 
		means3D.contiguous().data<float>(),
		...
		dL_dout_vfeature.contiguous().data_ptr<float>(),
		dL_dmeans2D.contiguous().data<float>(),
		dL_dconic.contiguous().data<float>(),  
		dL_dopacity.contiguous().data<float>(),
		dL_dcolors.contiguous().data<float>(),
		dL_dfeatures.contiguous().data_ptr<float>(),
		dL_dvfeatures.contiguous().data_ptr<float>(),
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dmeans3D, dL_dfeatures, dL_dvfeatures, dL_dcov3D, dL_dsh, dL_dscales, dL_drotations, dL_dviewmat, dL_dprojmat, dL_dcampos);
}
