...
__device__ float bilinearInterpolation(const float2& uv, const float2& p1, const float2& p2, const float2& p3, const float2& p4, float attr1, float attr2, float attr3, float attr4) 
{
	float u = uv.x;
	float v = uv.y;

	float attrTop = (1 - u) * attr1 + u * attr2;
	float attrBottom = (1 - u) * attr3 + u * attr4;
	float attrInterpolated = (1 - v) * attrTop + v * attrBottom;

	return attrInterpolated;
}

// __device__ float3 bilinearInterpolation3(const float2& uv, const float2& p1, const float2& p2, const float2& p3, const float2& p4, const float3 attr1, const float3& attr2, const float3& attr3, const float3& attr4) 
// {
// 	float u = uv.x;
// 	float v = uv.y;

// 	float3 attrTop = (1 - u) * attr1 + u * attr2;
// 	float3 attrBottom = (1 - u) * attr3 + u * attr4;
// 	float3 attrInterpolated = (1 - v) * attrTop + v * attrBottom;
	...
	float* config)
{
	...
	// 	lambda[idx] = { sqrt(lambda1), sqrt(lambda2) };
	// else
	// 	lambda[idx] = { sqrt(lambda2), sqrt(lambda1) };

	// lambda[idx] = {float(rect_max.x) -float(rect_min.x), float(rect_max.y) -float(rect_min.y)};

	lambda[idx] = {scaling.x, scaling.y};
	// pid[idx] = idx;
}
...
renderCUDA(
	...
	const int S, int VS, int W, int H, 
	const float* prcppoint,
	const float* patchbbox,
	const float focal_x, const float focal_y,
	const float2* __restrict__ points_xy_image,
	const float* __restrict__ features,
	const float* __restrict__ vfeatures,
	...
	float* __restrict__ out_vfeature,
	...
	float* config)
{
	...
	__shared__ float collected_color[CHANNELS * BLOCK_SIZE];
	...
	// __shared__ float collected_feature[24 * BLOCK_SIZE];
	...
	float F[50] = {0};
	float VF0[20] = {0}, VF1[20] = {0}, VF2[20] = {0}, VF3[20] = {0}, VF[20] = {0};

	float VF4[20] = {0}, VF5[20] = {0}, VF6[20] = {0}, VF7[20] = {0}, VF8[20] = {0};
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < CHANNELS; i++) collected_color[i * BLOCK_SIZE + block.thread_rank()] = colors[coll_id * CHANNELS + i];
			// for (int i = 0; i < S; i++) collected_feature[i * BLOCK_SIZE + block.thread_rank()] = features[coll_id * S + i];
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float w4 = 0.0f, w5 = 0.0f, w6 = 0.0f, w7 = 0.0f, w8 = 0.0f;
			...
			if (surface && per_pixel_depth) {
				...
				float inv_xx = con_o.x / det;
				float inv_yy = con_o.z / det;
				float inv_xy = -con_o.y / det;

				
				float dist_x = inv_xx * d.x * d.x;
				float dist_y = inv_yy * d.y * d.y;

				
				float m_x = sqrt(dist_x);
				float m_y = sqrt(dist_y);

				float2 lbd = collected_lambda[j];
				
				float max_dist_x = inv_xx * lbd.x * lbd.x;
				float max_dist_y = inv_yy * lbd.y * lbd.y;

				float m_max_x = 3* sqrt(max_dist_x);
				float m_max_y = 3*sqrt(max_dist_y);

				// float2 d_tan = {m_x, m_y};
				// float2 lbd_tan = {m_max_x, m_max_y};
				
				float2 d_tan = scr_to_tangent(d, Jinv_temp);
				// float2 lbd_tan = scr_to_tangent(lbd, Jinv_temp);
				float2 lbd_tan = lbd;

				float2 uv_max = { 0.5 * lbd_tan.x + 0.1, 0.5 * lbd_tan.y + 0.1};
				float2 uv = {d_tan.x / uv_max.x * 0.5f + 0.5f, d_tan.y / uv_max.y * 0.5f + 0.5f}; // normalize to [0, 1]
				
				uv.x = fminf(0.999f, fmaxf(0.001f, uv.x));
				uv.y = fminf(0.999f, fmaxf(0.001f, uv.y));

				w0 = (1.0f - uv.x) * (1.0f - uv.y);
				w1 = uv.x * (1.0f - uv.y);
				w2 = (1.0f - uv.x) * uv.y;
				w3 = uv.x * uv.y;
			}
			...
			for (int ch = 0; ch < CHANNELS; ch++) C[ch] += collected_color[ch * BLOCK_SIZE + j] * w;
			for (int ch = 0; ch < S; ch++)
				F[ch] += features[collected_id[j] * S + ch] * w;
			
			for (int ch = 0; ch < VS; ch+=4)
			{
				VF0[ch / 4] = vfeatures[collected_id[j] * VS + ch + 0] * w0;
				VF1[ch / 4] = vfeatures[collected_id[j] * VS + ch + 1] * w1;
				VF2[ch / 4] = vfeatures[collected_id[j] * VS + ch + 2] * w2;
				VF3[ch / 4] = vfeatures[collected_id[j] * VS + ch + 3] * w3;

				VF[ch / 4] += w * (VF0[ch / 4] + VF1[ch / 4] + VF2[ch / 4] + VF3[ch / 4]);
			}
			...
			T = test_T;
			atomicAdd(&(out_weights[collected_id[j]]), w);
	...
	if (inside)
	{
		...
		for (int ch = 0; ch < (VS / 4); ch++) {
			out_vfeature[ch * H * W + pix_id] = VF[ch];
		}
	...
	float* config)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> > (
		...
		n_contrib_cut,
		bg_color,
		out_color, out_normal, out_depth, out_opac, out_feature, out_vfeature, out_weights, //out_rayVar, 
		config);
}
