...
renderCUDA(
	...
	const float* __restrict__ dL_dpixfeature,
	const float* __restrict__ dL_dpixvfeature,
	...
	float* __restrict__ dL_dvfeature,
	float* config)
{
	...
	float accum_rec[C] = { 0 }, accum_rec_n[3] = {0}, accum_rec_d = 0, accum_rec_v = 0, accum_rec_f[50] = { 0 }, accum_rec_vf[80] = { 0 }; 
	float dL_dpixC[C], dL_dpixN[3], dL_dpixD, dL_dpixO, dL_dpixV;
	float dL_dpixF[50] = {0};
	float dL_dpixVF[20] = {0};

	if (inside) {
		for (int i = 0; i < C; i++) dL_dpixC[i] = dL_dpixcolor[i * H * W + pix_id];
		for (int i = 0; i < 3; i++) dL_dpixN[i] = dL_dpixnormal[i * H * W + pix_id];
		for (int i = 0; i < S; i++) dL_dpixF[i] = dL_dpixfeature[i * H * W + pix_id];
		for (int i = 0; i < VS/4; i++) dL_dpixVF[i] = dL_dpixvfeature[i * H * W + pix_id];
	...
	float last_feature[50] = { 0 };
	float last_vfeature[80] = { 0 };
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (surface && per_pixel_depth) {
				...
				float2 d_tan = scr_to_tangent(d, Jinv_u0_u1);
			...
			{
				// float weight[4] = {w0, w1, w2, w3};

				const float vfeature0 = vfeatures[global_id * VS + ch + 0] * w0;
				const float vfeature1 = vfeatures[global_id * VS + ch + 1] * w1;
				const float vfeature2 = vfeatures[global_id * VS + ch + 2] * w2;
				const float vfeature3 = vfeatures[global_id * VS + ch + 3] * w3;

				const float vfeature = vfeature0 + vfeature1 + vfeature2 + vfeature3;


				accum_rec_vf[ch/4] = last_alpha * last_vfeature[ch/4] + (1.f - last_alpha) * accum_rec_vf[ch/4];
				last_vfeature[ch/4] = vfeature;
				const float dL_dchannel_vf = dL_dpixVF[ch/4];

				atomicAdd(&(dL_dvfeature[global_id * VS + ch + 0]), w0 * dchannel_dcolor * dL_dchannel_vf);
				atomicAdd(&(dL_dvfeature[global_id * VS + ch + 1]), w1 * dchannel_dcolor * dL_dchannel_vf);
				atomicAdd(&(dL_dvfeature[global_id * VS + ch + 2]), w2 * dchannel_dcolor * dL_dchannel_vf);
				atomicAdd(&(dL_dvfeature[global_id * VS + ch + 3]), w3 * dchannel_dcolor * dL_dchannel_vf);

				dL_dalpha += (vfeature - accum_rec_vf[ch/4]) * dL_dchannel_vf;
			}
